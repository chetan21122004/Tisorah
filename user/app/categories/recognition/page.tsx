...
import { Heart, Search, Grid, List, Star, ArrowRight, Trophy, Award, ThumbsUp, Filter } from "lucide-react"
...
export default function RecognitionGiftsPage() {
  ...
  const [recognitionType, setRecognitionType] = useState("all")
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch category data
      const category = await getGiftCategoryBySlug('recognition')
      setCategoryData(category)
      
      // Fetch products
      const productsData = await getProductsByCategory('recognition')
      ...
      setProducts(productsData.map((product: Product) => ({
        ...
        delivery: product.delivery || "7-10 days",
        customizable: product.customizable || true,
        featured: product.featured || false,
        type: ["achievement", "service", "performance", "leadership", "innovation"][Math.floor(Math.random() * 5)]
      })))
  ...
  const benefits = [
    {
      ...
      title: "Boost Morale",
      description: "Recognize achievements and boost employee morale",
    },
    {
      icon: <Award className="w-6 h-6 text-amber-500" />,
      title: "Celebrate Success",
      description: "Celebrate milestones and professional accomplishments",
    },
    {
      icon: <ThumbsUp className="w-6 h-6 text-amber-500" />,
      title: "Retention Strategy",
      description: "Improve employee retention through meaningful recognition",
    },
  ]

  const filteredProducts = products.filter((product) => {
    const matchesSearch = product.name.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesType = recognitionType === "all" || product.type === recognitionType
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-amber-700 to-amber-900 text-white">
        <div className="container mx-auto px-4 py-16">
          <div className="max-w-3xl">
            <h1 className="text-4xl lg:text-5xl font-bold mb-4">Employee Recognition Gifts</h1>
            <p className="text-xl opacity-90 mb-8 leading-relaxed">
              {categoryData?.description || "Celebrate achievements and milestones with our premium recognition gifts, designed to show appreciation and motivate your team."}
            </p>
      ...
      <div className="container mx-auto px-4 py-16">
        {/* Benefits Section */}
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-gray-900 text-center mb-2">Why Recognition Matters</h2>
        ...
        <div className="mb-12">
          <div className="flex flex-col lg:flex-row justify-between items-center mb-6">
            <h2 className="text-3xl font-bold text-gray-900 mb-4 lg:mb-0">Recognition Gift Collection</h2>
